...
def get(platform_id: str):
    logging.info(f'Getting platforms. PlatformId: {platform_id}')
    return _create_response(get_platform_by_id(platform_id=platform_id))


@handler_exception
def put(platform_id: str, body: dict):
    logging.info(f'Updating platforms. PlatformId: {platform_id}')
...
def delete(platform_id: str):
    logging.info(f'Deleting platforms. PlatformId: {platform_id}')
